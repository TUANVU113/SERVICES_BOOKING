...
export async function PATCH(
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_BASE}/${id}/cancel`, {
      method: "PATCH",
      headers,
      ...
      return NextResponse.json(
        {
          message:
            data?.message ||
            `Hủy lịch đặt #${id} thất bại (Status: ${response.status})`,
        },
        { status: response.status }
      );
    }

    return NextResponse.json(data || { message: `Đã hủy lịch đặt #${id} thành công` });
  } catch (error: unknown) {
