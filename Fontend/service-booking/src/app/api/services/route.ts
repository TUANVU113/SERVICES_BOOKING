  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { message: data?.message || "Không thể lấy danh sách dịch vụ từ máy chủ" },
      ...
      return NextResponse.json(
        {
          message:
            ...
            (response.status === 403
              ...
              : `Tạo dịch vụ thất bại (Status: ${response.status})`),
        },
        { status: response.status }
      );
    }

    return NextResponse.json(data || { message: "Tạo dịch vụ thành công" });
  } catch (error: unknown) {
